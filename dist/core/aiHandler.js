 ...
 */
const processMessage = async (message, conversationHistory, businessId, currentActiveFlow, callSid) => {
    try {
        ...
        const determineNextVoiceAction = (intent, currentFlow, isEndingCall = false) => {
        ...
        if (currentActiveFlow?.startsWith('FAQ_CLARIFYING')) {
            ...
            const isClarificationClear = await isResponseClear(message, "FAQ clarification", "providing a clearer version of your question");
            if (!isClarificationClear) {
                // Still unclear - ask for clarification again or fall back
                const fallbackClarification = await generateClarifyingQuestion(message, "your question", "FAQ assistance", business.name);
                return {
        ...
        if (currentActiveFlow?.startsWith('LEAD_CAPTURE_CLARIFYING')) {
            ...
            // Check if the clarification response is now clear
            const isClarificationClear = await isResponseClear(message, `clarification for ${questionContext}`, `providing a clear answer to the ${questionContext} question`);
            if (!isClarificationClear) {
      ...
      Respond with only: IMMEDIATE or QUESTIONS`;
            const choiceResponse = await (0, openai_1.getChatCompletion)(escalationChoicePrompt, "You are an escalation preference detection expert.");
...
Classify as: LEAD_CAPTURE, FAQ, END_CALL, or OTHER`;
        ...
        if (intent === 'FAQ') {
            ...
            const isQuestionClear = await isResponseClear(message, "FAQ assistance", "asking a clear question that can be answered from our knowledge base");
            if (!isQuestionClear) {
                console.log('User question is unclear, asking for clarification...');
                const clarifyingQuestion = await generateClarifyingQuestion(message, "your question", "FAQ assistance - helping you find information", business.name);
                return {
...
User's Question: ${message}`;
            ...
            if (callSid) {
                try {
                    ...
                    const serviceOfInterest = await (0, openai_1.getChatCompletion)(serviceExtractionPrompt, "You are a service extraction expert. Extract only the main service or product mentioned.");
            ...
            if (isEmergency) {
                ...
                questionsToAsk = await selectSmartEmergencyQuestions(firstUserMessage, agentConfig.questions, business.name);
          ...
          Respond with only the number (1-10).`;
                    try {
                        const severityResponse = await (0, openai_1.getChatCompletion)(severityCheckPrompt, "You are an emergency severity assessment expert.");
            ...
            for (let i = 0; i < conversationHistory.length - 1; i++) {
                ...
                if (entry.role === 'assistant') {
                    ...
                    if (matchedQuestion && nextEntry && nextEntry.role === 'user') {
                        ...
                        const isAnswerClear = await isResponseClear(nextEntry.content, `answer to: ${matchedQuestion.questionText}`, `providing a clear answer to the question about ${matchedQuestion.questionText.toLowerCase()}`);
                        if (!isAnswerClear) {
                            console.log(`Answer "${nextEntry.content}" to question "${matchedQuestion.questionText}" is unclear, asking for clarification`);
                            const clarifyingQuestion = await generateClarifyingQuestion(nextEntry.content, matchedQuestion.questionText, "lead capture - collecting your information", business.name);
...
Generate a natural way to ask this question that flows well in the conversation.`;
                    const rawQuestionResponse = await (0, openai_1.getChatCompletion)(questionPrompt, enhancedSystemPrompt);
                ...
                if (nextQuestion?.mapsToLeadField === 'address') {
                    const confirmationResponse = await confirmEmergencyDetails(message, 'address', business.name);
                    return {
                ...
                if (nextQuestion?.mapsToLeadField === 'contactPhone') {
                    ...
                    if (callerId) {
                        const confirmationResponse = await confirmEmergencyDetails(message, 'phone', business.name);
                        return {
                            reply: confirmationResponse,
                ...
                if (nextQuestion?.mapsToLeadField === 'contactName') {
                    const confirmationResponse = await confirmEmergencyDetails(message, 'name', business.name);
                    return {
                        reply: confirmationResponse,
...
User's message: ${message}`;
            const rawResponse = await (0, openai_1.getChatCompletion)(generalChatUserPrompt, systemPrompt);
            const cleanedResponse = rawResponse ? cleanVoiceResponse(rawResponse) : null;
            return {
        ...
        if (currentActiveFlow?.startsWith('LEAD_CAPTURE_CONFIRM_')) {
            ...
            if (!isConfirmed) {
                ...
                const retryQuestion = agentConfig?.questions?.find((q) => q.mapsToLeadField === questionType);
